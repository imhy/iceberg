...
public class TestFlinkFilters {
  ...
  static Stream<Arguments> timestampLiteralMismatches() {
    LocalDateTime dateTime = LocalDateTime.parse("2020-12-23T12:13:14");
    Instant instant = Instant.parse("2020-12-23T12:13:14.00Z");
    return Stream.of(false, true)
        .flatMap(
            reversed ->
                Stream.of(
                    // Instant literal against a TIMESTAMP (without time zone) field
                    Arguments.of("field11", instant, reversed),
                    // LocalDateTime literal against a TIMESTAMP_LTZ field
                    Arguments.of("field12", dateTime, reversed),
                    // timestamp literals against a STRING field
                    Arguments.of("field5", dateTime, reversed),
                    Arguments.of("field5", instant, reversed)));
  }

  @ParameterizedTest
  @MethodSource("timestampLiteralMismatches")
  void doesNotPushDownMismatchedTimestampLiteral(
      String fieldName, Object literal, boolean reversed) {
    Expression expr =
        resolve(
            reversed
                ? Expressions.lit(literal).isEqual(Expressions.$(fieldName))
                : Expressions.$(fieldName).isEqual(Expressions.lit(literal)));
    assertThat(FlinkFilters.convert(expr)).isNotPresent();
  }
